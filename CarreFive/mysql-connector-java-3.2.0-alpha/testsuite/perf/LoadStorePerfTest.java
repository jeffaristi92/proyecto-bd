...
public class LoadStorePerfTest extends BasePerfTest {
    ...
    public void setUp() throws Exception {
        ...
        for (int i = 0; i < numLoops; i++) {
        	this.stmt.executeUpdate("INSERT INTO perfLoadStore (" + "priKey, " + "fk1, "
    ...
    protected void doOneIteration() throws Exception {
        PreparedStatement pStmtStore = this.conn.prepareStatement(
        ...
        PreparedStatement pStmtCheck = this.conn.prepareStatement(
                "SELECT COUNT(*) FROM perfLoadStore WHERE priKey=?");
        PreparedStatement pStmtLoad =  null;
        
        if (this.largeResults) {
        	pStmtLoad = this.conn.prepareStatement("SELECT "
