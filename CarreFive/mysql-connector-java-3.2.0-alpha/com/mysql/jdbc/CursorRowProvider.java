 ...
 * 
 * @version $Id: CursorRowProvider.java,v 1.1.2.4 2004/12/13 22:22:06 mmatthew Exp $
 */
public class CursorRowProvider implements RowProvider {
	...
	/**
	 * Where we are positionaly in the entire result set, used mostly
	 * to facilitate easy 'isBeforeFirst()' and 'isFirst()' methods.
	 */
	private int currentPositionInEntireResult = BEFORE_START_OF_ROWS;
	
	/**
	 * Position in cache of rows, used to determine if we need to fetch
	 * more rows from the server to satisfy a request for the next row.
	 */
	...
	/**
	 * Field-level metadata from the server. We need this, because it is
	 * not sent for each batch of rows, but we need the metadata to unpack
	...
	/**
	 * The server status for 'last-row-sent'...This might belong
	 * in mysqldefs, but it it only ever referenced from here.
	 */
    ...
    public boolean isAfterLast() {
        return lastRowFetched && this.currentPositionInFetchedRows > this.fetchedRows.size();
    }
    ...
    public boolean hasNext() throws SQLException {
        ...
        if (this.currentPositionInEntireResult != BEFORE_START_OF_ROWS) {
        	...
        	if (this.currentPositionInFetchedRows < (this.fetchedRows.size() - 1)) {
        		return true;
        	} else if (this.currentPositionInFetchedRows == this.fetchedRows.size() &&
        			this.lastRowFetched) {
        		return false;
        	} else {
	...
	private void fetchMoreRows() throws SQLException {
		...
		synchronized (this.owner.connection.getMutex()) {
			...
			int numRowsToFetch = this.prepStmt.getFetchSize();
