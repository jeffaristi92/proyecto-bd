...
public class TimeUtil {
    ...
    public static Timestamp changeTimezone(Connection conn, Timestamp tstamp,
        ...
        if ((conn != null) && conn.getUseTimezone()) {
            ...
            if (rollForward || conn.isServerTzUTC()) {
            	toTime += offsetDiff;
            } else {
            	toTime -= offsetDiff;
            }
    ...
    //
    final static Date fastDateCreate(Calendar cal, int year, int month, int day) {
        cal.clear();
    ...
    final static Timestamp fastTimestampCreate(Calendar cal, int year,
