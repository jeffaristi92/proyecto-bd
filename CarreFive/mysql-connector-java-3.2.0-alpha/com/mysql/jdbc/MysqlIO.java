 ...
 * @version $Id: MysqlIO.java,v 1.32.4.81.2.21 2004/12/13 22:22:06 mmatthew Exp $
 *
...
class MysqlIO {
    ...
    private RowProvider streamingData = null;
    ...
    protected ResultSet getResultSet(Statement callingStatement,
    	...
    	//
    	// Handle cursor-based case first
    	//
    	if (this.connection.versionMeetsMinimum(5, 0, 2) &&
    		...
    		RowProvider rows = new CursorRowProvider(this, prepStmt, 
    				((com.mysql.jdbc.ResultSetMetaData)prepStmt.getMetaData()).fields);
    		
    		return buildResultSetWithRows(callingStatement, catalog, ((com.mysql.jdbc.ResultSetMetaData)prepStmt.getMetaData()).fields,
        ...
        RowProvider rowData = null;

        boolean dontUnpack = this.connection.getDontUnpackBinaryResults();
        
        if (!streamResults) {
            rowData = readSingleRowSet(columnCount, maxRows,
                    resultSetConcurrency, isBinaryEncoded, fields, dontUnpack);
        } else {
            rowData = new DirectRowProvider(this, (int) columnCount, fields,
                    isBinaryEncoded);
        ...
        rs.setBinaryDataIsUnpacked(!dontUnpack);
    ...
    protected List fetchRowsViaCursor(List fetchedRows, long statementId, Field[] columnTypes, int fetchSize) throws SQLException {
    	...
    	sendCommand(MysqlDefs.COM_FETCH, null, this.sharedSendPacket, true, null);
    	
    	Object[] row = null;
    	
    	while ((row = nextRow(columnTypes, 
    			columnTypes.length, 
				true, 
				ResultSet.CONCUR_READ_ONLY, 
    			this.connection.getDontUnpackBinaryResults())) != null) {
    		fetchedRows.add(row);
    	}
    ...
    */
    final Object[] nextRow(Field[] fields, int columnCount,
        boolean isBinaryEncoded, int resultSetConcurrency, boolean dontUnpack)
        throws SQLException {
        ...
        if (!rowPacket.isLastDataPacket()) {
            return unpackBinaryResultSetRow(fields, rowPacket,
                resultSetConcurrency, dontUnpack);
        }
    ...
    void closeStreamer(RowProvider streamer) throws SQLException {
    ...
    private com.mysql.jdbc.ResultSet buildResultSetWithRows(
        ...
        com.mysql.jdbc.Field[] fields, RowProvider rows, int resultSetType,
    ...
    private RowProvider readSingleRowSet(long columnCount, int maxRows,
        int resultSetConcurrency, boolean isBinaryEncoded, 
		Field[] fields, boolean dontUnpack)
        throws SQLException {
        RowProvider rowData;
        ...
        rowData = new InMemoryRowProvider(rows);
    ...
    */
    ...
    private final Object[] unpackBinaryResultSetRow(Field[] fields,
        Buffer binaryData, int resultSetConcurrency, boolean dontUnpack) throws SQLException {
            ...
            if ((nullBitMask[nullMaskPos] & bit) != 0) {
            	...
            	if (dontUnpack) {
            		extractNativeEncodedColumn(binaryData, fields, i, 
            				unpackedRowData, resultSetConcurrency);
            	} else {
    ...
    private final void unpackNativeEncodedColumn(Buffer binaryData, 
        ...
        case MysqlDefs.FIELD_TYPE_TINY:
            ...
            if (!curField.isUnsigned()) {
                if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                    unpackedRowData[columnIndex] = new Integer(tinyVal); // req'd by Table B-3 in JDBC-3.0 spec
                } else {
                ...
                if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                    unpackedRowData[columnIndex] = new Short(unsignedTinyVal);
                } else {
        ...
        case MysqlDefs.FIELD_TYPE_YEAR:
            ...
            if (!curField.isUnsigned()) {
                if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                    unpackedRowData[columnIndex] = new Integer(shortVal); // req'd by Table B-3 in JDBC-3.0 spec
                } else {
                ...
                if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                    unpackedRowData[columnIndex] = new Integer(unsignedShortVal);
                } else {
        ...
        case MysqlDefs.FIELD_TYPE_INT24:
            ...
            if (!curField.isUnsigned()) {
                if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                    unpackedRowData[columnIndex] = new Integer(intVal);
                } else {
                ...
                if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                    unpackedRowData[columnIndex] = new Long(longVal);
                } else {
        ...
        case MysqlDefs.FIELD_TYPE_LONGLONG:
            ...
            if (!curField.isUnsigned()) {
                ...
                if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                    unpackedRowData[columnIndex] = asBigInteger;
                } else {
        ...
        case MysqlDefs.FIELD_TYPE_FLOAT:
            ...
            if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                unpackedRowData[columnIndex] = new Float(floatVal);
            } else {
        ...
        case MysqlDefs.FIELD_TYPE_DOUBLE:
            ...
            if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                unpackedRowData[columnIndex] = new Double(doubleVal);
            } else {
        ...
        case MysqlDefs.FIELD_TYPE_TIME:
            ...
            if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                unpackedRowData[columnIndex] = TimeUtil.fastTimeCreate(getCalendarInstanceForSessionOrNew(),
                        hour, minute, seconds);
            } else {
        ...
        case MysqlDefs.FIELD_TYPE_DATE:
            ...
            if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                unpackedRowData[columnIndex] = TimeUtil.fastDateCreate(getCalendarInstanceForSessionOrNew(),
                        year, month, day);
            } else {
        ...
        case MysqlDefs.FIELD_TYPE_TIMESTAMP:
            ...
            if (resultSetConcurrency == java.sql.ResultSet.CONCUR_READ_ONLY) {
                unpackedRowData[columnIndex] = TimeUtil.fastTimestampCreate(getCalendarInstanceForSessionOrNew(),
                ...
                for (int j = 0; columnIndex < nanosAsBytes.length; j++) {
